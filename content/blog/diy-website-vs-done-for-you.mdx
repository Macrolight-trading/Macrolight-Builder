...
For a practical example of how local intent pages should be structured, see our [Web Design in Ferndale, MI](/blog/web-design-ferndale-mi).

## How do you compare the total cost of DIY and done-for-you web design?

**Compare the work required over the next 12 months, not just the platform invoice.** A simple decision worksheet should include software, setup, owner hours, content and media production, maintenance, and the value of leads delayed while the site is unfinished. A done-for-you quote should be judged against the same categories rather than against a DIY subscription alone.

Use this framework before choosing a path:

| Cost question | DIY path | Done-for-you path |
| --- | --- | --- |
| What is the cash cost? | Platform, domain, plugins, templates, and paid add-ons | Project fee or subscription, plus any excluded tools |
| Who owns the strategy? | The owner or an internal employee | The provider, with owner input and approvals |
| Who creates the revenue pages? | The owner must plan, write, and publish them | The provider should scope service, location, and conversion pages |
| What happens after launch? | The owner schedules fixes, updates, and testing | Confirm exactly what support and iteration are included |
| How will success be checked? | The owner must connect Search Console and lead data | Confirm the reporting, tracking, and handoff process |

This is a planning model, not a promise that one delivery model always wins. Google says its systems prioritize helpful, reliable, people-first content, and that SEO is useful when applied to people-first content rather than to search-engine-first writing. So ask each provider — including yourself — how the work will make the site more useful to the customer, not merely how many pages or keywords will be delivered. ([Google Search Central: Creating helpful content](https://developers.google.com/search/docs/fundamentals/creating-helpful-content))
