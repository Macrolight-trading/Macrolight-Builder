...
import type { ContentPlanWithMeta } from "@/lib/audit/ai/content-plan";
import GenerateContentPlanButton from "@/components/admin/GenerateContentPlanButton";
    ...
    <>
      ...
      {result && (
        <>
          ...
          {/* AI Content Plan — generated on-demand by gpt-4o, replaces the
              rule-based 90-day roadmap. Persists once generated. */}
          <ContentPlanSection
            auditId={audit.id}
            plan={readContentPlan(result)}
            generatedAt={(result as { aiContentPlanAt?: Date | null }).aiContentPlanAt ?? null}
          />
...
/** Pull the AI-generated content plan out of the result row, with type guard. */
function readContentPlan(
  result: { aiContentPlan?: unknown } | null
): ContentPlanWithMeta | null {
  if (!result) return null;
  const raw = result.aiContentPlan;
  if (!raw || typeof raw !== "object") return null;
  // Trust the persisted shape — generated by Zod-validated AI output.
  return raw as ContentPlanWithMeta;
}
  ...
  return (
    <section className="mb-6 bg-white rounded-xl border border-gray-200 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-100 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-sm font-semibold text-gray-900">Content Plan</h2>
          <p className="mt-0.5 text-xs text-gray-500">
            AI-generated specific page recommendations grounded in this
            audit&apos;s data.
            {plan && generatedAt && (
              <>
                {" "}Generated {new Date(generatedAt).toLocaleString()} ·{" "}
                <span className="text-gray-400">{plan.model}</span>
              </>
            )}
          </p>
        </div>
        <GenerateContentPlanButton auditId={auditId} hasPlan={!!plan} />
      </div>
      {plan ? (
        <div className="px-5 py-5">
          <p className="mb-5 text-sm text-gray-700 italic">
            ...
            {plan.recommendations.map((rec, idx) => (
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex items-center gap-2">
                    <span className="inline-block w-6 h-6 rounded-full bg-violet-100 text-violet-700 font-bold text-xs flex items-center justify-center">
                      {idx + 1}
                    </span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3 text-xs">
                  <KvTile label="Target keyword" value={rec.targetKeyword} />
                  <KvTile
                    label="Search volume"
                    value={
                      rec.searchVolumePerMonth != null
                        ? `${rec.searchVolumePerMonth.toLocaleString()} / mo`
                        : "—"
                    }
                  />
                  {rec.improvementTargetUrl ? (
                    <KvTile
                      label="Existing URL"
                      value={rec.improvementTargetUrl}
                      mono
                    />
                  ) : (
                    <KvTile label="Type" value={typeLabel(rec.type)} />
                  )}
                </div>
                ...
                {rec.internalLinkingNote && (
                  <p className="mt-2 text-xs text-gray-500">
                    <strong className="font-medium text-gray-600">Internal linking:</strong>{" "}
                    {rec.internalLinkingNote}
                  </p>
                )}
              </li>
        ...
        <div className="px-5 py-12 text-center">
          <p className="text-sm text-gray-500">
            No Content Plan yet. Click <strong className="text-gray-700">Generate Content Plan</strong> to
            produce 5–10 specific page recommendations from this audit&apos;s data.
          </p>
        </div>
      )}
    </section>
...
function PriorityChip({ priority }: { priority: "high" | "medium" | "low" }) {
  ...
  return (
    <span className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide ${styles[priority]}`}>
      {priority}
    </span>
  );
}

function TypeChip({ type }: { type: "blog_post" | "service_page" | "landing_page" | "improvement" }) {
  return (
    <span className="inline-flex items-center rounded border border-violet-200 bg-violet-50 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-violet-700">
      {typeLabel(type)}
    </span>
  );
}

function typeLabel(t: "blog_post" | "service_page" | "landing_page" | "improvement"): string {
  switch (t) {
...
function KvTile({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="rounded border border-gray-200 bg-white px-2.5 py-1.5">
      ...
      <p className={`text-xs font-medium text-gray-800 truncate ${mono ? "font-mono" : ""}`} title={value}>
        {value}
      </p>
    </div>
  );
